...
- When a task involves tapping, swiping, describing, or capturing the iOS Simulator UI, call the `axe` skill (AXe CLI) available at `skills/axe/SKILL.md`. Architects, packagers, and testers should especially rely on it for screenshots, recordings, and UI inspections so simulator captures stay consistent.
- Provide the target simulator UDID, app bundle, and any selectors/labels the skill should use before invoking it so the automation steps stay focused on the right screen.
